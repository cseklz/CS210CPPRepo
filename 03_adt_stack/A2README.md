...
### Refection Questions
1. `topIndex` starts at `-1`. Since valid indexes begin at `0`, `-1` means there is no top element. This lets `isEmpty()` check whether `topIndex == 1` without special-casing for a `null` stack.
...
3. None of the functions loop through any data and only consist of a few simple operations. Thus, the time complexity for every function is **O(1)**.
